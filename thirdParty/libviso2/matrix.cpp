...
void Matrix::setVal(FLOAT s,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  ...
  if (j2==-1) j2 = n-1;
  if (i2<i1 || j2<j1) {
    std::cerr << "ERROR in setVal: Indices must be ordered (i1<=i2, j1<=j2)." << std::endl;
    exit(0);
  }
  for (int32_t i=i1; i<=i2; i++)
    for (int32_t j=j1; j<=j2; j++)
...
Matrix Matrix::diag (const Matrix &M) {
  ...
  std::cout << "ERROR: Trying to create diagonal matrix from vector of size (" << M.m << "x" << M.n << ")" << std::endl;
  exit(0);
}
...
//    std::ostream &operator<<(std::ostream &out, const libviso2::Matrix &M) {
//        if (M.m == 0 || M.n == 0) {
//            out << "[empty matrix]";
//        } else {
//            char buffer[1024];
